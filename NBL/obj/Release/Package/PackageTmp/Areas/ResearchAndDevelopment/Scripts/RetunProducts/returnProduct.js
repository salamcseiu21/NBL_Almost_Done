...
function ConfirmToReturn(btnClick) {
    ...
    if (isConfirm) {
        window.location.href = RootUrl + "ResearchAndDevelopment/product/ConfirmReturnEntry";
    }
    return false;
}
...
function RemoveAll() {
    ...
    if (result) {
        ...
        $.ajax({
            type: 'POST',
            url: RootUrl + 'ResearchAndDevelopment/product/RemoveAll',
...
function ShowDeliveryDetails(event) {
    ...
    $.ajax({
        type: "POST",
        url: RootUrl + 'ResearchAndDevelopment/product/DeliveryDetailsByDeliveryId',
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify(json),
        success: function (data) {
